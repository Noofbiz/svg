...
func setupViewBox(vb string) error {
	...
	width, err := getSVGLength(vbDim[2], width)
	if err != nil {
		return err
	}

	height, err := getSVGLength(vbDim[3], height)
	if err != nil {
		return err
	}
	...
	viewBoxWidth = svgLength{float64(width.convert(1) - x0.convert(1)), px}
	viewBoxHeight = svgLength{float64(height.convert(1) - y0.convert(1)), px}
